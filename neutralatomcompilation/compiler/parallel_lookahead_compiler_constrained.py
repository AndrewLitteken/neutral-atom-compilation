...
class LookaheadCompilerConstrainedParallel(Compiler):
    ...
    def _initial_mapping(self,
        ...
        self.viable_nodes_sets = self.hardware.get_non_overlapping_constrained_qubit_sets(len(circuit.qubits))
    ...
    def compile(self,
        ...
        if l2p is None:
            print("no mapping")
            return None
        new_circuit = self._schedule_and_route(dag, l2p, p2l, lookahead_distance, weighting_function, dist_dict, frontier)
        ...
        for j, constrained_set in enumerate(self.viable_nodes_sets):
            for i, qubit in enumerate(constrained_set):
                hq_qiskit = self.hardware.hardware_to_qiskit[self.viable_nodes_sets[0][i]]
        ...
        number_parallel_at_once = int(np.ceil(len(self.viable_nodes_sets) * self.percentage_parallel))
        if self.percentage_parallel == 0:
            number_parallel_at_once = 1
        separation_index = int(np.ceil(len(self.viable_nodes_sets) / number_parallel_at_once))
        ...
        while current_index < len(self.viable_nodes_sets):
            ...
            self.included_constrained_sets.append((constrained_list, len(included_sets), included_sets))
